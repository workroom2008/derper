#!/usr/bin/env python3
"""替换 DERP Web UI 模板为中文美化版"""
import re, sys

target = sys.argv[1] if len(sys.argv) > 1 else "build_src/tailscale/cmd/derper/derper.go"

NEW_TEMPLATE = r'''// homePageTemplate renders the home page using [templateData].
...
# 找到 homePageTemplate 块: 从注释开始到 `)) 结束
pattern = r'// homePageTemplate renders the home page.*?\`\)\)'
new_content, count = re.subn(pattern, NEW_TEMPLATE, content, count=1, flags=re.DOTALL)
...
print(f"OK: template replaced in {target}")
